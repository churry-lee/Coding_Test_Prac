...
def solution(monster, S1, S2, S3):
    rolls = product(range(1, S1+1), range(1, S2+1), range(1, S3+1))
    total_combinations = S1*S2*S3
    pos_counter = Counter(sum(x) for x in list(rolls))
    monster_chances = sum(pos_counter[monster_pos-1] for monster_pos in monster)
    return int(((total_combinations-monster_chances)*1000/total_combinations))
